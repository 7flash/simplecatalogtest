								...
								<ul class="field-list js-field-list">
									{foreach from=$oScheme->getFields() item=oField}
										<li title="{$oField->getDescription()|escape:'html'}"{if $oField@iteration % 2 == 0} class="second"{/if}>
											<span class="fl-r">
												<a href="{$oField->getEditWebPath()}" class="sc-icon-edit" title="{$aLang.plugin.simplecatalog.Edit}"></a>
												<a href="{$oField->getDeleteWebPath()}" class="sc-icon-remove js-question" title="{$aLang.plugin.simplecatalog.Delete}"></a>
											</span>
