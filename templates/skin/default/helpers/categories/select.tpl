	...
	*}
	...
	<select name="{$sName}{if $bMultipleSelect}[]{/if}"
			class="{if $sSelectClassName}{$sSelectClassName}{else}input-text input-width-250{/if}"
			...
			{foreach from=$aCategoryTree item=aCategoryItem}
				...
				<option value="{$oCategory->getId()}"
					...
					{if $oCategory->getItemsCount()}
						({$oCategory->getItemsCount()} {$aLang.plugin.simplecatalog.Categories.items_in_category})
					{/if}
				</option>
			{/foreach}
		{/if}
