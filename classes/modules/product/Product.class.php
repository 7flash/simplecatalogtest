...
class PluginSimplecatalog_ModuleProduct extends ModuleORM {
	...
	public function RemoveProjectImages($arParams) {
		...
		$this->oMapper->RemoveProjectImages($arParams);
	}
	 ...
	 * @return string
	 */
	public function MyGetValidAndFreeProductUrl($sProductUrl, $aProductFields, $oProduct, $scheme = "") {
		$sUrl = null;
		/*
		...
		if ($scheme == Config::Get("getfunded.project_code")) {
			// если другой продукт с таким урлом уже есть
			if ($oFoundProduct = $this->MyGetProductByProductUrl($sUrl) and $oFoundProduct->getId() != $oProduct->getId()) {
				return array(
					"res" => "fail",
					"duplicate" => 1
				);
	...
	public function sanitizeEditorImages($arParams) {
		if (!isset($arParams["html"][0])) {
			return "";
		}

		$doc = new DOMDocument();
		$doc->loadHTML('<meta http-equiv="Content-Type" content="text/html; charset=utf-8">' . $arParams["html"]);
		$arRawImages = $doc->getElementsByTagName('img');

		$arWorkImages = array();
		foreach($arRawImages as $img) {
			$arWorkImages[] = $img;
		}

		foreach ($arWorkImages as $image) {
			// Если картинка из директории данного сайта
			if (strpos($image->getAttribute('src'), "/uploads/") === 0) {

				$width = $image->getAttribute('width');

				// Если у картинки есть атрибут ширины
				if ($width) {

					/*
					// Установим ей нужный класс
					$setClassWidth = "js_setwidth_" . $width;
					// Если ширина картинки > максимальной -> установим ей максимальную
					if ($width > Config::Get("getfunded.editor_img_width_max")) {
						$setClassWidth = "js_setwidth_" . Config::Get("getfunded.editor_img_width_max");
					}

					$classInit = $image->getAttribute('class');

					$image->setAttribute('class', $classInit . " " . $setClassWidth);
					*/

					// Если ширина картинки > максимальной -> установим ей максимальную
					if ($width > Config::Get("getfunded.editor_img_width_max")) {
						$image->setAttribute('width', Config::Get("getfunded.editor_img_width_max"));
					}
				}

			// Картинку со стороннего ресурса удаляем
			} else {
		...
		$html_clean = preg_replace(
			array('/^<!DOCTYPE.+?>/i', '/<head>s*(.*?)<\/head>/i'),
			'',
			str_replace(
				array('<html>', '</html>', '<body>', '</body>'), array('', '', '', ''),
				$doc->saveHTML()
			)
		);
