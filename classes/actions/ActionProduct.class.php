...
class PluginSimplecatalog_ActionProduct extends ActionPlugin {
	...
	public $arProjectFormTabs = array(
		"main" => array(
			"projectname" => 1,
			"previewtext" => 1,
			"sum" => 1,
	...
	protected function RegisterEvent() {
		 ...
		 * общие эвенты
		 */
		//$this->AddEventPreg('#^add[^/project]$#', 'EventAdd');
		//$this->AddEventPreg('#^add#', '#^project$#', 'EventAddProject');
		$this->AddEventPreg('#^add$#', 'EventAdd');

		$this->AddEventPreg('#^save$#', 'EventSave');
		// SubmitProjectSave
	...
	public function EventAdd() {
		...
		if ($oScheme->getSchemeUrl() == Config::Get("getfunded.project_code")) {
			$this->Viewer_AddHtmlTitle("Редактирование проекта", array('scheme' => $oScheme->getSchemeName()));
		} else {
		...
		if (isPost('submit_add') or isPost('submit_save_draft')) {
			...
			// Если это project
			if ($oScheme->getSchemeUrl() == Config::Get("getfunded.project_code")) {
				return $this->SubmitProjectCreate($oScheme);
			} else {
	 ...
	 * @throws Exception
	 */
	private function SubmitProjectCreate($oScheme) {

		// TODO - поставить ограничение на количество создаваемых проектов одним юзером
		...
		if (!$this->oUserCurrent or !$this->oUserCurrent->getCanAddNewProductsInScheme($oScheme)) {
			$this->Message_AddError($this->Lang('Errors.Access_Denied'), $this->Lang_Get('error'), true);
			Router::Location(Config::Get('path.root.web') . Config::Get('getfunded.path.page_rules'));
		}

		if (!isset($_POST["agree"][0])) {
			$this->Message_AddError($this->Lang('Errors.gf_project_cannot_create'), $this->Lang_Get('error'), true);
			Router::Location(Config::Get('path.root.web') . Config::Get('getfunded.path.page_rules'));
		}
		...
		if (!$oProduct->_Validate()) {
			$this->Message_AddError($oProduct->_getValidateError(), $this->Lang_Get('error'), true);
			Router::Location(Config::Get('path.root.web') . Config::Get('getfunded.path.page_rules'));
		}
		...
		if ($aCategoriesIds = (array) getRequest('categories_ids') and ($mCategoriesValidation = $this->PluginSimplecatalog_Product_CheckCategoriesIdsAreCorrect($aCategoriesIds)) !== true) {
			$this->Message_AddError($mCategoriesValidation, $this->Lang_Get('error'), true);
			Router::Location(Config::Get('path.root.web') . Config::Get('getfunded.path.page_rules'));
		}
		...
		if ($mHookSuccess !== true) {
			$this->Message_AddError($mHookSuccess, $this->Lang_Get('error'), true);
			Router::Location(Config::Get('path.root.web') . Config::Get('getfunded.path.page_rules'));
		}

		////////////////////////////////////////////////////
		// START - Сохранение продукта в БД
		////////////////////////////////////////////////////
	...
	public function EventSave() {
		...
		// Если страница запрошена НЕ через $_POST -> редирект на страницу проектов
		if (!isset($_POST["id"])) {
			Router::Location(Config::Get('path.root.web') . Config::Get('getfunded.path.page_project_index'));
		}
		...
		// получить схему из урла
		if (!$oScheme = $this->PluginSimplecatalog_Scheme_MyGetActiveSchemeBySchemeUrl((string) $this->GetParam(0))) {
			$obAjax->showResult(json_encode(array("res"  => "fail", "mes_error" => $this->Lang('Errors.Scheme_Not_Found'))));
		}

		// проверить авторизацию пользователя
		if (!$this->oUserCurrent) {
			$obAjax->showResult(json_encode(array("res"  => "fail", "mes_error" => $this->Lang('Errors.Access_Denied'))));
		}
		$this->sMenuHeadItemSelect = $oScheme->getSchemeUrl();
		...
		// если была отправлена форма
		// if (isPost('submit_add') or isPost('submit_save_draft')) {
		if (isPost('submit_save_draft')) {
			//$this->Security_ValidateSendForm();
			if (!$this->Security_ValidateSessionKey()) {
				$obAjax->showResult(json_encode(array("res"  => "fail", "mes_error" => $this->Lang_Get('gf_error.unable_perform_action'))));
			}
			return $this->SubmitProjectSave($oScheme);
		}
	}

	private function SubmitProjectSave($oScheme) {

		$obAjax = new Gf_Ajax();

		$this->SetTemplateAction('project_add');
		$this->Viewer_AddHtmlTitle("Редактирование проекта");
		...
		if ($iProductIdRaw = (int) getRequest('id')) {
			if (!$oProduct = $this->PluginSimplecatalog_Product_MyGetProductById($iProductIdRaw)) {
				$obAjax->showResult(json_encode(array("res"  => "fail", "mes_error" => $this->Lang('Errors.Product_Not_Found'))));
			}
			...
			if (!$this->oUserCurrent or !$this->oUserCurrent->getCanManageProduct($oProduct)) {
				$obAjax->showResult(json_encode(array("res" => "fail",
				                                      "mes_error" => $this->Lang('Errors.You_Cant_Edit_This_Product'))));
			}
		} else {
			$oProduct = Engine::GetEntity('PluginSimplecatalog_Product');
			// проверить права пользователя для создания нового продукта
			if (!$this->oUserCurrent or !$this->oUserCurrent->getCanAddNewProductsInScheme($oScheme)) {
				$obAjax->showResult(json_encode(array("res" => "fail",
				                                      "mes_error" => $this->Lang('Errors.Access_Denied'))));
			}
		}
		...
		// Текущая вкладка формы
		$tabCode = trim($_POST["form_tab_code"]);

		// пройтись по всем полям схемы для получения значений полей продукта
		foreach($aFields as $oField) {

			if (!isset($this->arProjectFormTabs[$tabCode][$oField->getCode()])) {
				continue;
			}
			...
			if (is_null($mValue)) {
				$obAjax->showResult(
					json_encode(
						array("res"       => "fail",
						      "mes_error" => $this->Lang('Errors.Wrong_Product_Request_Data', array('field' => $oField->getTitle())))
					)
				);
			...
			if ($bCanBeParsedAsText) {
				...
				// Если работа с 1-й вкладкой
				if ($tabCode == "main") {

					// Название проекта
					if ($oField->getCode() == "projectname") {
						if (!preg_match('/^[A-Za-zА-Яа-яЁё0-9_"\s-]+$/iu', $mValue)) { // Неверный набор символов
							$obAjax->showResult(
								json_encode(
									array("res"       => "fail",
									      "mes_error" => $this->Lang('Errors.Field_Not_Correct', array('field' => $oField->getTitle())))
								)
							);
						}
					}

					if ($oField->getCode() == "dateuntil") {
						// Если дата окончания проекта меньше текущей даты
						if(strtotime($mValue) <= strtotime("now")) {
							$obAjax->showResult(json_encode(array("res" => "fail", "mes_error" => $this->Lang_Get('gf_error.dateuntil_wrong'))));
						}
					}
				// Если работа со 2-й вкладкой
				} elseif ($tabCode == "detail") {

					if ($oField->getCode() == "videolink" && isset($mValue[0])) {
						if (!preg_match("/^https:\/\/www\.youtube\.com\/watch\?v=[a-zA-Z0-9‌​_\-]{11}$/i", $mValue)) {
							$obAjax->showResult(
								json_encode(
									array("res"       => "fail",
									      "mes_error" => $this->Lang('Errors.Field_Not_Correct', array('field' => $oField->getTitle())))
					...
					if ($oField->getCode() == "detailtext" ) {

						$mValue = str_replace(
							array('style="text-align: center;"', 'style="text-align:center;"', 'style="text-align:center"', 'style="text-align: center"',
							      'style="text-align: right;"', 'style="text-align:right;"', 'style="text-align:right"', 'style="text-align: right"'
							),
							array('class="text_align_center"', 'class="text_align_center"', 'class="text_align_center"', 'class="text_align_center"',
							      'class="text_align_right"', 'class="text_align_right"', 'class="text_align_right"', 'class="text_align_right"'
							),
							$mValue
						);

						// Проверяем все картинки в тексте
						$mValue = $this->PluginSimplecatalog_Product_sanitizeEditorImages(array("html" => $mValue));

						// TODO - отключена проверка оябзательности поля: Детальное описание
						/*
						if (!isset($mValue[0])) {
							$obAjax->showResult(
								json_encode(
				...
				if (($mCustomValidatorCheck = $this->PluginSimplecatalog_Product_CheckValueByCustomFieldValidator($oField, $mValue)) !== true) {
					$obAjax->showResult(json_encode(array("res" => "fail",
					                                      "mes_error" => $mCustomValidatorCheck)));
				}
			}
			...
			if (!$oEnt->_Validate()) {
				$obAjax->showResult(json_encode(array("res" => "fail", "mes_error" => $oEnt->_getValidateError())));
			}
		...
		$arResProductUrl = array();
		// Проверяем url проекта только на главной вкладке
		if ($tabCode == "main") {

			$arResProductUrl = $this->PluginSimplecatalog_Product_MyGetValidAndFreeProductUrl(
				// урл, созданный пользователем
				getRequest('product_url'),
				// или для получения его на основе данных полей
				$aProductFields[PluginSimplecatalog_ModuleProduct::PRODUCT_FIELDS_SAVE_FIRST_QUEUE],
				// или из продукта
				$oProduct,
				Config::Get("getfunded.project_code")
			);

			// Если проекта с таким url не существует -> обновляем его url
			if ($arResProductUrl["res"] == "ok") {
				// ЧПУ
				$oProduct->setProductUrl($arResProductUrl["url"]);
			// Вывод сообщения, что url занят
			} else {
				$obAjax->showResult(json_encode(array("res" => "fail", "mes_error" => "Данный <strong>Адрес проекта</strong> уже занят")));
			}
		}
		...
		// валидация внесенных основных данных для продукта
		if (!$oProduct->_Validate()) {
			$obAjax->showResult(json_encode(array("res" => "fail", "mes_error" => $oProduct->_getValidateError())));
		}

		// Проверяем Категории проекта только на главной вкладке
		if ($tabCode == "main") {
			// получение массива ид категорий и их проверка на существование и корректность согласно правилам (на наличие дочерних категорий и т.п.)
			if ($aCategoriesIds = (array) getRequest('categories_ids')
				and ($mCategoriesValidation = $this->PluginSimplecatalog_Product_CheckCategoriesIdsAreCorrect($aCategoriesIds)) !== true
			) {
				$obAjax->showResult(json_encode(array("res" => "fail", "mes_error" => $mCategoriesValidation)));
			}
		}
		...
		if ($mHookSuccess !== true) {
			$obAjax->showResult(json_encode(array("res" => "fail", "mes_error" => $mHookSuccess)));
		}
		...
		// теперь в $oProduct уже есть новый id (орм устанавливает для примари ключей с автоинкрементом)
		if (!$oProduct->Save()) {
			$obAjax->showResult(json_encode(array("res" => "fail", "mes_error" => $this->Lang('Errors.system_error_when_save_product'))));
		}
			...
			if ($this->PluginSimplecatalog_File_IsFileUploaded($oProductField->getContent())) {
				...
				if (!$mValue = $this->PluginSimplecatalog_File_UploadContentFile($oProductField->getContent())) {
					$obAjax->showResult(
						json_encode(
							array("res" => "fail",
							      "mes_error" => $this->Lang('Errors.System_Error_With_File_Upload', array('title' => $oProductField->getField()->getTitle())))
						)
					);
				}
		...
		$arRes = array(
			"res" => "ok",
			"mes_success" => $this->Lang_Get('gf_success.data_saved')
		);

		if ($tabCode == "main" && isset($arResProductUrl["url"][0])) {
			$arRes["product_url"] = $arResProductUrl["url"];
			$arRes["product_url_full"] = $oProduct->getItemShowWebPathDir();
		}

		$obAjax->showResult(json_encode($arRes));
	}
	...
	public function EventEdit() {
		...
		if ($oScheme->getSchemeUrl() == Config::Get("getfunded.project_code")) {
			...
			$this->Viewer_Assign('arProjectFormTabs', $this->arProjectFormTabs);
		} else {
		...
		// для отображения статусов проекта
		$_REQUEST['moderation_done'] = $oProduct->getModerationDone();
		...
		if ($oScheme->getSchemeUrl() == Config::Get("getfunded.project_code")) {
			...
			// Находим картинки проекта
			if ($oProduct->getImageDetail() && $oProduct->getImagePreview()) {
				...
				if (!empty($aFoundImages)) {
					$arProjectImages = array();
					foreach($aFoundImages as $arImage) {
						if ($arImage["id"] == $oProduct->getImagePreview()) {
							$arProjectImages["image_preview"]["id"] = $oProduct->getImagePreview();
							$arProjectImages["image_preview"]["url"] = $arImage["file_path"];
						} elseif ($arImage["id"] == $oProduct->getImageDetail()) {
							$arProjectImages["image_detail"]["id"] = $oProduct->getImageDetail();
							$arProjectImages["image_detail"]["url"] = $arImage["file_path"];
						}
					}

					$this->Viewer_Assign('arProjectImages', $arProjectImages);
				}
			}

			// <script src="//cdn.ckeditor.com/4.5.10/standard/ckeditor.js"></script>
			// TODO - проверить подключение
			//$this->Viewer_AppendScript('//cdn.ckeditor.com/4.5.10/full/ckeditor.js', array("merge" => false));

			$this->Viewer_AppendScript('/engine/lib/external/ckeditor/ckeditor.js', array("merge" => false));

			$this->Viewer_AppendScript(Plugin::GetTemplateWebPath(__CLASS__) . 'js/project_edit.js');

			$this->Viewer_AppendScript(Plugin::GetTemplateWebPath(__CLASS__) . 'js/project_images.js');
			$this->Viewer_AppendScript(Plugin::GetTemplateWebPath(__CLASS__) . 'js/ajaxformnew.js');

			$this->Viewer_AppendScript('/engine/lib/external/fancybox/jquery.fancybox.pack.js', array("merge" => false));
			$this->Viewer_AppendStyle('/engine/lib/external/fancybox/jquery.fancybox.css');
